...
func (l *ArticlesLogic) Articles(in *pb.ArticlesRequest) (*pb.ArticlesResponse, error) {
	...
	if len(articleIds) > 0 {
		...
		if len(articles) > int(in.PageSize) { //如果返回的数量大于一页的数量，则切出要一页的数量来
			firstPageArticles = articles[:int(in.PageSize)]
		} else { //如果返回的数量小于一页的数量，则说明不足一页，到最后了
			firstPageArticles = articles
			isEnd = true
		}
		for _, article := range firstPageArticles { //复制当前页
			curPage = append(curPage, &pb.ArticleItem{
				Id:              article.Id,
				Title:           article.Title,
				Content:         article.Content,
	...
	if !isCache {
		threading.GoSafe(func() { //异步写入缓存
			if len(articles) < types.DefaultLimit && len(articles) > 0 { //最后一页，用 id=-1来表示
