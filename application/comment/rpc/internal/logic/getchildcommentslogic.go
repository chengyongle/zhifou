...
// 获取评论的全部子评论列表
func (l *GetChildCommentsLogic) GetChildComments(in *service.GetChildCommentsRequest) (*service.GetChildCommentsResponse, error) {
	...
	if len(commentIds) > 0 {
		...
		v, err, _ := l.svcCtx.SingleFlightGroup.Do(fmt.Sprintf("CommentsByID:%d:%d:%d", in.BizId, in.ObjId, in.SortType), func() (interface{}, error) {
			return l.svcCtx.CommentModel.FindChildCommentsByBizIdAndObjId(l.ctx, in.BizId, in.ObjId, in.ParentId, types.CommentStatusVisible, sortLikeNum, sortCreateTime, sortField, types.DefaultLimit)
		})
		if err != nil {
		...
		if v == nil {
			return &service.GetChildCommentsResponse{}, nil
		}
	...
	ret := &service.GetChildCommentsResponse{
		Comments: curPage,
		BizId:    in.BizId,
		IsEnd:    isEnd,
		Cursor:   cursor,
...
// 并行处理
func (l *GetChildCommentsLogic) commentsByIds(ctx context.Context, commentIds []int64) ([]*model.Comment, error) {
...
func (l *GetChildCommentsLogic) cacheComments(ctx context.Context, bizId string, objId, parentId, cursor, pagesize int64, sortType int32) ([]int64, error) {
	key := getCommentsKey(bizId, types.ChildComments, objId, parentId, sortType)
...
func (l *GetChildCommentsLogic) addCacheComments(ctx context.Context, bizId string, objId, parentId int64, sortType int32, comments []*model.Comment) error {
	if len(comments) == 0 {
		return nil
	}
	key := getCommentsKey(bizId, types.ChildComments, objId, parentId, sortType)
