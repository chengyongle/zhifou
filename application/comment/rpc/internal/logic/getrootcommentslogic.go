...
// 获取帖子的根评论列表
func (l *GetRootCommentsLogic) GetRootComments(in *service.GetRootCommentsRequest) (*service.GetRootCommentsResponse, error) {
	...
	if len(commentIds) > 0 {
		...
		v, err, _ := l.svcCtx.SingleFlightGroup.Do(fmt.Sprintf("CommentsByID:%d:%d:%d", in.BizId, in.ObjId, in.SortType), func() (interface{}, error) {
			return l.svcCtx.CommentModel.FindRootCommentsByBizIdAndObjId(l.ctx, in.BizId, in.ObjId, types.CommentStatusVisible, sortLikeNum, sortCreateTime, sortField, types.DefaultLimit)
		})
		if err != nil {
		...
		if v == nil {
			return &service.GetRootCommentsResponse{}, nil
		}
	...
	ret := &service.GetRootCommentsResponse{
		Comments: curPage,
		BizId:    in.BizId,
		IsEnd:    isEnd,
		Cursor:   cursor,
...
// 并行处理
func (l *GetRootCommentsLogic) commentsByIds(ctx context.Context, commentIds []int64) ([]*model.Comment, error) {
...
func (l *GetRootCommentsLogic) cacheComments(ctx context.Context, bizId string, objId, parentId, cursor, pagesize int64, sortType int32) ([]int64, error) {
	key := getCommentsKey(bizId, types.RootComments, objId, parentId, sortType)
...
func (l *GetRootCommentsLogic) addCacheComments(ctx context.Context, bizId string, objId, parentId int64, sortType int32, comments []*model.Comment) error {
	if len(comments) == 0 {
		return nil
	}
	key := getCommentsKey(bizId, types.RootComments, objId, parentId, sortType)
