...
var (
	...
	cacheZhifouCommentCommentIdPrefix = "cache:zhifouComment:comment:id:"
)
...
type (
	...
	Comment struct {
		...
		ObjId           int64    `db:"obj_id"`             // id
		CommentUserId   int64    `db:"comment_user_id"`    // ID
		BeCommentUserId int64    `db:"be_comment_user_id"` // ID
		ParentId        int64    `db:"parent_id"`          // ID0
...
func (m *defaultCommentModel) Delete(ctx context.Context, id int64) error {
	zhifouCommentCommentIdKey := fmt.Sprintf("%s%v", cacheZhifouCommentCommentIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, zhifouCommentCommentIdKey)
	return err
}
...
func (m *defaultCommentModel) FindOne(ctx context.Context, id int64) (*Comment, error) {
	zhifouCommentCommentIdKey := fmt.Sprintf("%s%v", cacheZhifouCommentCommentIdPrefix, id)
	var resp Comment
	err := m.QueryRowCtx(ctx, &resp, zhifouCommentCommentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultCommentModel) Insert(ctx context.Context, data *Comment) (sql.Result, error) {
	zhifouCommentCommentIdKey := fmt.Sprintf("%s%v", cacheZhifouCommentCommentIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?)", m.table, commentRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.BizId, data.ObjId, data.CommentUserId, data.BeCommentUserId, data.ParentId, data.Content, data.Status, data.LikeNum)
	}, zhifouCommentCommentIdKey)
	return ret, err
}
...
func (m *defaultCommentModel) Update(ctx context.Context, data *Comment) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.BizId, data.ObjId, data.CommentUserId, data.BeCommentUserId, data.ParentId, data.Content, data.Status, data.LikeNum, data.Id)
	}, zhifouCommentCommentIdKey)
	return err
}

func (m *defaultCommentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheZhifouCommentCommentIdPrefix, primary)
}
